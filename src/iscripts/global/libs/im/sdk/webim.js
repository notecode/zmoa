...
(function (webim) {
    ...
    var ajaxRequest = function (meth, url, req, timeout, isLongPolling, cbOk, cbErr) {
        ...
        if(url.indexOf('group_msg_get') > 0){
            url = api.url+'?r=im/im01';
            var reqJson = JSON.parse(req);
            var newReq = {
                group_id: reqJson.GroupId,
                msg_id_max: reqJson.ReqMsgSeq,
                pagesize: reqJson.ReqMsgNumber
            };
            if(!reqJson.ReqMsgSeq){
                req = 'group_id='+ reqJson.GroupId+'&pagesize='+reqJson.ReqMsgNumber;
            }else{
                req = 'group_id='+ reqJson.GroupId+'&msg_id_max='+reqJson.ReqMsgSeq+'&pagesize='+reqJson.ReqMsgNumber;
            }
        }
    ...
    var MsgManager = new function () {
        ...
        var handlerOrdinaryAndTipGroupMsgs = function (eventType, groupMsgArray) {
            ...
            for (var j=0,jj=groupMsgArray.length; j<jj; j++) {
        ...
        var handlerGroupSystemMsgs = function (groupSystemMsgs, isNeedValidRepeatMsg) {
            for (var k=0,l=groupSystemMsgs.length; k<l; k++) {
        ...
        this.longPolling = function (cbOk, cbErr) {
            ...
            proto_longPolling(opts, function (resp) {
                if(resp.EventArray){
                    for (var i=0;i<resp.EventArray.length;i++) {
