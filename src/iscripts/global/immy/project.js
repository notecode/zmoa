...
(function () {
    ...
    $(document).ready(function () {
        var guestToken = is_wph() ? '70e53d5531e0a42fe2c879419848deb8' : 'fde1aab0afa2589bbf2e3144ce3029a8'; 
        /*
        a_profile_req({
            succ: function (user) {
        ...
        */
        ...
        // 登录后（不刷新页面）调用，暂先给www调用（下同）
        project.loginIMAsLoggedInUser = function () {
            webimConfigs.logout();
            webimConfigs.login(project.data.user, {
                succ: function () {}
            });
        }

        // 退出后（不刷新页面）调用
        project.loginIMAsGuest = function () {
            var user = {
                user_id: '1',
                nim_user_token: guestToken,
                user_type: '0'
